...
class FolderRuleModal extends Modal {
	...
	onOpen() {
				...
				if (prop.type === "checkbox") {
					...
					setting.addText((cb) => {
						...
						if (prop.type === "datetime")
							cb.inputEl.type = "datetime-local";
		...
		new Setting(contentEl)
			.addButton((bt) =>
				bt.setButtonText("Add property").onClick(() => {
							...
							.catch((err) => {
								const message =
									err instanceof Error
										...
										: String(err);
								console.error(
									"Folder Auto Properties: Failed to save inside modal context",
									message,
								);
							});
					}),
						...
						try {
							...
							const message =
								err instanceof Error
									...
									: String(err);
							console.error(
								"Folder Auto Properties: Async create execution failure",
								message,
							);
						} finally {
					...
					for (const rule of matchingRules) {
						for (const prop of rule.properties) {
							...
							if (prop.type === "tags" || prop.type === "list") {
								const newItems =
									typeof valToSet === "string"
										? this.parseTags(valToSet)
										: [];
...
class FolderAutoPropertiesSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Define properties for a specific folder. Rules apply to new notes only.",
			)
			.addButton((btn) =>
				btn
